...
static int CONV cmd_eking_value(tree_t * restrict ptree);
#endif
...
#endif
#if defined(ENTERING_KING)
  if (!strcmp(token, "eking_value")) { return cmd_eking_value(ptree); }
#endif
...
init_usi_option(void)
{
	...
	strncpy(usi_value.time_control, "Time", BUFSIZ);
	usi_value.time_a = 0;
	usi_value.time_b = 5;
	...
	usi_value.resign = 4000;
	usi_value.memory = 512;
...
static int CONV proce_usi(tree_t * restrict ptree)
{
	...
	if (!strcmp(token, "usi"))
	{
		...
		USIOut("option name %s type combo default %d var 2000 var 3000 var 4000 var 4500 var 5000 var 32596\n", usi_name.resign, usi_value.resign);
		USIOut("option name %s type combo default %d var 16 var 32 var 64 var 128 var 256 var 512 var 768 var 1024 var 1536 var 2048 var 3072 var 4096\n", usi_name.memory, usi_value.memory);
...
cmd_usrmove(tree_t * restrict ptree, const char *str_move, char **lasts)
{
	...
	if (str == NULL) { lelapsed = 0; }
		...
		if (ptr == str || lelapsed == LONG_MAX || lelapsed < 0)
		{
			str_error = str_bad_cmdline;
			return -2;
		}
...
static int CONV cmd_eking_value(tree_t * restrict ptree)
 {
   Out("eking_value is black %d, white %d\n", eking_value(ptree, 0), eking_value(ptree, 1));
   return 1;
 }
#endif
