...
FLAG = /DNDEBUG /DTLP /DHAVE_SSE4 /DMPV /DCSA_LAN /DMNJ_LAN
...
cl-pgo:
	$(MAKE) -f Makefile_d1.vs clean
	$(MAKE) -f Makefile_d1.vs bonanza.exe CC="cl" LD="link"\
		CFLAGS="$(FLAG) /MT /W4 /nologo /O2 /Ob2 /GS- /GL"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG:PGI"
	$(MAKE) -f Makefile_d1.vs pgo-run
	del bonanza.exe
	$(MAKE) -f Makefile_d1.vs bonanza.exe LD="link"\
...
pgo-run:
	echo learn no-ini 32 -1 11 4 4  >> runprof
