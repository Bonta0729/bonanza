...
FLAG = /DNDEBUG /DMINIMUM /DTLP /DHAVE_SSE4 /DUSI /DNO_LOGGING /DMPV /ENTERING_KING_BONUS /DCSA_LAN /DMNJ_LAN
...
cl:
	$(MAKE) -f Makefile_release_usi_ek.vs bonanza.exe CC="cl" LD="link"\
		CFLAGS="$(FLAG) /MT /W4 /nologo /O2 /Ob2 /GS- /GL"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG"

cl-pgo:
	$(MAKE) -f Makefile_release_usi_ek.vs clean
	$(MAKE) -f Makefile_release_usi_ek.vs bonanza.exe CC="cl" LD="link"\
		CFLAGS="$(FLAG) /MT /W4 /nologo /O2 /Ob2 /GS- /GL"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG:PGI"
	$(MAKE) -f Makefile_release_usi_ek.vs pgo-run
	del bonanza.exe
	$(MAKE) -f Makefile_release_usi_ek.vs bonanza.exe LD="link"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG:PGO"

icl:
	$(MAKE) -f Makefile_release_usi_ek.vs bonanza.exe CC="icl" LD="icl"\
