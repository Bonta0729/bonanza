...
# /DENTERING_KING    enables entering king related codes
# /DLEARN_EKING      learn only the entering king parameter 
# /DENTERING_KING_BONUS  


FLAG = /DNDEBUG /DMINIMUM /DTLP /DHAVE_SSE4 /DCSASHOGI /DNO_LOGGING /DENTERING_KING_BONUS 
...
cl:
	$(MAKE) -f Makefile_release_nyugyoku.vs bonanza.exe CC="cl" LD="link"\
		CFLAGS="$(FLAG) /MT /W4 /nologo /O2 /Ob2 /GS- /GL"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG"

cl-pgo:
	$(MAKE) -f Makefile_release_nyugyoku.vs clean
	$(MAKE) -f Makefile_release_nyugyoku.vs bonanza.exe CC="cl" LD="link"\
		CFLAGS="$(FLAG) /MT /W4 /nologo /O2 /Ob2 /GS- /GL"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG:PGI"
	$(MAKE) -f Makefile_release_nyugyoku.vs pgo-run
	del bonanza.exe
	$(MAKE) -f Makefile_release_nyugyoku.vs bonanza.exe LD="link"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG:PGO"

icl:
	$(MAKE) -f Makefile_release_nyugyoku.vs bonanza.exe CC="icl" LD="icl"\
