...
FLAG = /DNDEBUG /DMINIMUM /DTLP /DHAVE_SSE4 /DUSI /DNO_LOGGING /DMPV 
...
cl:
	$(MAKE) -f Makefile_release_usi.vs bonanza.exe CC="cl" LD="link"\
		CFLAGS="$(FLAG) /MT /W4 /nologo /O2 /Ob2 /GS- /GL"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG"

cl-pgo:
	$(MAKE) -f Makefile_release_usi.vs clean
	$(MAKE) -f Makefile_release_usi.vs bonanza.exe CC="cl" LD="link"\
		CFLAGS="$(FLAG) /MT /W4 /nologo /O2 /Ob2 /GS- /GL"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG:PGI"
	$(MAKE) -f Makefile_release.vs pgo-run
	del bonanza.exe
	$(MAKE) -f Makefile_release_usi.vs bonanza.exe LD="link"\
		LDFLAGS="/NOLOGO /out:bonanza.exe /LTCG:PGO"

icl:
	$(MAKE) -f Makefile_release_usi.vs bonanza.exe CC="icl" LD="icl"\
